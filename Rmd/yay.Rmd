...
ptype_dns_records_netlify <-
  ...
  dplyr::select(-target) |>
  tibble::add_column(id = character(),
                     dns_zone_id = character(),
                     site_id = character(),
                     managed = logical())

ptype_dns_records_porkbun <-
  ptype_dns_records |>
  tibble::add_column(id = character())
```
...
-   [Record types supported by Netlify](https://docs.netlify.com/domains-https/netlify-dns/dns-records/#supported-record-types).

    Custom `NETLIFY`/`NETLIFY6` record types give HTTP 401 Unauthorized, thus excluded.
...
as_dns_records <- function(records,
  ...
  ptype_dns_records_registrar <- get(paste0("ptype_dns_records_", registrar))
  
  # ensure all required cols are present
  present_cols <- intersect(colnames(records),
                            colnames(ptype_dns_records_registrar))
  
  if (length(present_cols) > 0L) {
    records %<>% dplyr::left_join(y = ptype_dns_records_registrar,
                                  by = present_cols)
  } else {
    records %<>% dplyr::cross_join(y = ptype_dns_records_registrar)
  }
  ...
  tibble::as_tibble(records) |>
    ...
    dplyr::relocate(any_of(colnames(ptype_dns_records_registrar)))
}
```
...
-   Ideally, we'd use an established standard as universal input/output format for the `netlify_*()` and `porkbun_*()` DNS record handling functions
    (data-schema-wise).

    The current industry standard is the [DNS zone file format](https://en.wikipedia.org/wiki/Zone_file#File_format). It is not very structured, i.e. it encodes
    a record via only four semantic fields `type`, `hostname`, `ttl` and `value` – whereas the latter comprises data we want to have further itemized.

    With [RFC 8427](https://datatracker.ietf.org/doc/rfc8427/) there's an informational IETF standard to represent DNS data in JSON. It is built around the
    concept of DNS *messages* and uses a quite complex, verbose schema to represent individual records (encoding most of it in `RDATAHEX` values) that is
    neither easily comprehensible, nor particularly suited to convert data to tabular form.

    A more suitable alternative could be the [DNS YAML](https://hugot.nl/projects/dns-yml/index.html) format. It is similar to (but entails more semantics
    than[\^1]) the format [`bacon`](https://github.com/jungaretti/bacon) uses, a CLI written in Go to back up and restore Porkbun DNS records from/to YAML
    ...
    A third format is the [YAML format supported by Google's `gcloud` CLI](https://cloud.google.com/dns/docs/records#import_and_export_record_sets). But
    semantically it just seems to be a less intuitive alteration of Porkbun's data format.

    For now, we refrain from supporting any of the above and instead adopt and extend Netlify's format (which already offers a bit more universally applicable
    semantics than Porkbun's).

### DESCRIPTION

Functions to work with [Netlify's RESTful API](https://docs.netlify.com/api/get-started/).
...
#'
#' @return `r pkgsnip::param_lbl("tibble_cols", cols = colnames(ptype_dns_records_netlify))`
#' @family netlify
#' @export
#'
#' @examples
...
#' `r colnames(ptype_dns_records_netlify) |> intersect(colnames(ptype_dns_records)) |> pal::enum_str(wrap = "\x60")`. Further columns are silently ignored.
#'
...
#' `r pkgsnip::param_lbl("tibble_cols", cols = colnames(ptype_dns_records_netlify), as_sentence = FALSE) |> pal::capitalize_first()`, invisibly.
#' @family netlify
#' @export
#'
#' @examples
    ...
    purrr::pmap(.progress = TRUE,
                .f = \(type,
                  ...
                  httr2::request(base_url = glue::glue("https://api.netlify.com/api/v1/dns_zones/{zone_id}/dns_records/")) |>
                    ...
                    # canonicalize key order
                    dplyr::relocate(any_of(colnames(ptype_dns_records_netlify))) |>
                    dplyr::arrange(type, hostname, value)
                }) |>
    purrr::list_rbind() |>
    invisible()
...
Functions to work with [Porkbun's API](https://porkbun.com/api/json/v3/documentation).

### `porkbun_dns_records_get`

NOTES:

-   There are two more API endpoints to retrieve individual or per-subdomain/type subsets of a domain's DNS records. They aren't implemented (yet).
...
#'
#' @return  `r pkgsnip::param_lbl("tibble_cols", cols = colnames(ptype_dns_records_porkbun))`
#' @family porkbun
#' @export
#'
#' @examples
  ...
  perform_porkbun_req(url = glue::glue("https://api.porkbun.com/api/json/v3/dns/retrieve/{domain}"),
    ...
    dplyr::rename_with(.cols = everything(),
                       .fn = \(x) dplyr::case_match(x,
                                                    ...
                                                    .default = x)) %>%
    # normalize cols
    dplyr::mutate(dplyr::across(.cols = any_of(intersect(colnames(dplyr::select(.data = ptype_dns_records_porkbun,
                                                                                where(is.integer))),
                                                         colnames(.))),
                                .fns = as.integer)) |>
...
#' `r colnames(ptype_dns_records_porkbun) |> intersect(colnames(ptype_dns_records)) |> pal::enum_str(wrap = "\x60")`. Further columns are silently ignored.
#'
#' @details
...
#' @inheritParams netlify_dns_records_set
#' @param records DNS records. A dataframe/tibble with the columns
#'   `r colnames(ptype_dns_records_porkbun) |> intersect(colnames(ptype_dns_records)) |> pal::enum_str(wrap = "\x60")`. The first three columns are mandatory,
#'   columns not listed here are silently ignored.
...
-   There's another API endpoint to delete records by type and optionally subdomain. It isn't implemented (yet).
