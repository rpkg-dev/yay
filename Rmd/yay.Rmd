...
#' `yay.local_pkgdown_deploy_paths`. The value of this option must be a named character vector whereas the names correspond to the package names and the values
...
#'   `yay.local_pkgdown_deploy_paths`, which must be a named character vector of the form `c(pkg_name = "/path/to/local_git_repo")`.
...
deploy_pkgdown_site <- function(pkg_path = ".",
  ...
  if (is.null(to_path)) {
    ...
    if (is.null(to_path) || is.na(to_path)) {
      rlang::abort(glue::glue("Set `to_path` either directly to a valid filesystem path or provide the path in the R option `yay.local_pkgdown_deploy_paths` ",
