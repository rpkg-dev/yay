...
### Regular expression rules for text normalization
...
regex_text_normalization <-
  ...
  tibble::add_row(id = "no_break_percentages",
                  ...
                  pattern = list("(\\d+) ?(\\\\?%|Prozent|percent|per\\scent)(?![\"'])"),
                  replacement = "\\1\u202f\\2") %>%
...
### Regular expression rules for file normalization
...
-   [ ] Create a function based upon [this StackOverflow answer](https://stackoverflow.com/a/15794201/7196903).
...
deploy_pkgdown_site <- function(pkg_path = ".",
  ...
  if (is.null(to_path)) {
    ...
    to_path <- getOption("yay.local_pkgdown_deploy_paths")[config$package]
    
    if (is.null(to_path) || is.na(to_path)) {
      
      to_path_parent <- getOption("yay.default_local_pkgdown_deploy_parent_path")
      
      if (is.null(to_path_parent)) {
        rlang::abort(glue::glue("Either set `to_path` directly to a valid filesystem path or provide the path in the R option `yay.local_pkgdown_deploy_paths`",
                                " as a named character vector of the form `c({config$package} = \"/path/to/local_git_repo\")`. Another alternative is to set a",
                                " default parent path in the R option `yay.default_local_pkgdown_deploy_parent_path`. See `?yay::deploy_pkgdown_site` for",
                                " details."))
        
      } else if (!checkmate::test_directory(to_path_parent,
                                            access = "w")) {
        rlang::abort(glue::glue("The R option `yay.default_local_pkgdown_deploy_parent_path` is set, but not to valid filesystem path with write access.",
                     " Current option value: {to_path_parent}"))
        
      } else {
        to_path <- fs::path(to_path_parent, config$package)
      }
    }
  }
...
-   [ ] Investigate if we can somehow consolidate console output per line and pattern. When a single pattern yields multiple replacement infos, the console
    output is currently confusing. Example:
