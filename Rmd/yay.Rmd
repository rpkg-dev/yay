...
#' `r pal::as_md_vals(netlify_dns_record_cols$col) |> cli::ansi_collapse(last = " and ")`. Further columns are silently ignored.
#'
#' Supported are the DNS record types `netlify_dns_record_types |> pal::wrap_chr("\x60") |> cli::ansi_collapse( last = " and ")`. `A` and
...
#' @param domain The domain name to set DNS records for. This is translated into the corresponding Netlify DNS Zone. A character scalar.
#' @param records A dataframe/tibble with the columns
#'   `r pal::as_md_vals(netlify_dns_record_cols$col) |> cli::ansi_collapse(last = " and ")`. Further columns are silently ignored.
#' @param token Personal access token used for authentication.
#'
#' @return A list of response objects, invisibly.
#' @family netlify
#' @export
#'
#' @examples
...
#'                                token  = "XYZ1234")}
netlify_dns_records_set <- function(domain,
                                    records,
                                    token,
                                    max_tries = 3L) {
  ...
  checkmate::assert_data_frame(records,
                               row.names = "unique")
  missing_col_names <- setdiff(netlify_dns_record_cols$col,
                               colnames(records))
  ...
  purrr::walk(netlify_dns_record_cols$col[1:3],
              \(x) {
  ...
  records |>
    ...
    dplyr::mutate(dplyr::across(.cols = everything(),
                                .fns = \(x) {
                                  .Primitive(paste0("as.",
                                                    netlify_dns_record_cols$type[netlify_dns_record_cols$col == dplyr::cur_column()]))(x)
                                })) |>
    purrr::pmap(\(type,
                  hostname,
                  value,
